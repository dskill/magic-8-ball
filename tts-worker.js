...
async function synthesize(text, voiceId = 'M1') {
    ...
    const speakerEmbeddings = embeddings[voiceId] || embeddings.F1;
    ...
    try {
        ...
        // Result contains { audio: Float32Array, sampling_rate: number }
        self.postMessage({
            status: 'complete',
            audio: result.audio,
...
async function load() {
    self.postMessage({
        status: 'loading',
        data: 'Loading TTS model from HuggingFace...',
    });
    ...
    try {
        ...
        // Warm up with a short phrase to compile WebGPU shaders
        self.postMessage({
            status: 'loading',
